...
const controlValue = function(pValue) {
    if (pValue) {
        return true;
    } else {
        throw new Error('empty function result');
    }
};
...
const compose = function(...functions) {
    ...
    for (let i = functions.length - 1; i >= 0; i--) {
        try {
            controlFunction(functions[i]);
            controlValue(functions[i](allText));
            allText = functions[i](allText);
        } catch (err) {
            console.log(err);
            return;
        }
